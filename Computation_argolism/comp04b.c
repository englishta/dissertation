...
#define X0  -5  /* x軸の左端(最小値) */
#define X1  15  /* x軸の右端(最大値) */
...
int main( int argc, char *argv[] )
{
  ...
  xb = 20.0;  /* 初期区間の設定(終点) */
  xc = ( xa + xb ) / 2.0;  /* 中点の計算 */
  printf( "初期区間: [xa, xb] = [%.15e, %.15e]\n", xa, xb );
  while( fabs( f( xc ) ) > eps ) {  /* 終了判定条件(条件式が満たされる限りwhile文を継続) */
    if( f( xa ) * f( xc ) < 0 ) xb = xc; /* 前半分の区間に変更 */
    else                        xa = xc; /* 後半分の区間に変更 */
    xc = ( xa + xb ) / 2.0;  /* 中点の再計算 */
    n++;
    printf( "%d回目の区間: [xa, xb] = [%.15e, %.15e]\n", n, xa, xb );
  }
  x_solution = xc;    /* 最終的な解の値をグローバル変数に代入 */
  printf( "\n最終的なf(x)=0の解\n" );
