...
int main ( void )
{
  ...
  for( t = t0; t <= t_end; t += dt ) {
    ...
    k2 = f( a, c, t + 0.5 * dt, x + 0.5 * k1 ) * dt;  /* 2段目(中点1での傾きを利用) */
    k3 = f( a, c, t + 0.5 * dt, x + 0.5 * k2 ) * dt;  /* 2段目(中点2での傾きを利用) */
    k4 = f( a, c, t + dt, x + k3 ) * dt;              /* 4段目(終点での傾きを利用) */
